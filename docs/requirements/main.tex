...
Die Verwendung von (\textit{WK}) außerhalb des Blocks \textbf{Zielbestimmung} weist darauf hin, dass es sich bei der Funktion um ein Wunschkriterium handelt.
...
Es ist eine Open Source Veröffentlichung geplant.
...
\begin{figure}[H]%
  \centering
  \includegraphics[width=15cm]{img/use-case/UseCaseDiagramFinal2NWM.jpg}
  \caption{Anwendungsfalldiagramm}
\end{figure} 
\newpage
\subsection{Funktionsübersicht}
\startcontents[toc:functionoverview]
\printcontents[toc:functionoverview]{}{3}{}
%\newpage
...
\begin{itemize}
    ...
    \begin{itemize}
        \item[2a.] Nutzer bricht den Vorgang ab.
    \end{itemize}
    \item[\underline{Kriterien:}] /MK120/
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item[1b.] Optional kann die CSV-Datei auch nicht lokal in der Projektdatei als Kopie angelegt werden. Dies kann aus datenschutzrechtlichen Gründen verwendet werden.
    \end{itemize}
    \item[\underline{Kriterien:}] /MK120/
\end{itemize}
...
\begin{itemize}
    ...
    \item[\underline{Erweiterung:}]
    \begin{itemize}
        \item[2a.] Besitzt eine andere Attributsableitung bereits den Namen, weist eine Fehlermeldung darauf hin. Der Name kann anschließend geändert werden.
        \item[3a.] Beinhaltet die Attributsableitung einen Syntax- oder Typfehler, weist eine Fehlermeldung den Nutzer darauf hin und die Ableitung wird als invalide markiert. 
    \end{itemize}
...
\begin{itemize}
    ...
    \item[\textbf{/LK20/}] Invalide Nutzenfunktionen werden visuell und mit Fehlermeldung markiert.
    \item[\textbf{/LK30/}] Invalide Attributsableitungen werden visuell und mit Fehlermeldung markiert.
    \item[\textbf{/LK40/}] Das Speichern einer Projektdatei mit invaliden Nutzenfunktionen oder invaliden Attributsableitungen wird dem Nutzer mit einer Fehlermeldung angemerkt. Danach kann der Nutzer den Speichervorgang fortführen oder abbrechen.
    \item[\textbf{/LK50/}] Das Laden einer Projektdatei mit invaliden Nutzenfunktionen oder invaliden Attributsableitungen wird dem Nutzer mit einer Fehlermeldung angemerkt.
    \item[\textbf{/LK60/}] Das Exportieren einer CSV-Datei bei Existenz von invaliden Attributsableitungen wird dem Nutzer mit einer Fehlermeldung angemerkt. Danach kann der Nutzer den Vorgang fortführen oder abbrechen.
\end{itemize}
...
\begin{figure}[H]%
  \centering
  \includegraphics[width=12cm]{img/gui-screenshots/splitter1.png}
...
\begin{figure}[H]%
  \centering
  \includegraphics[width=12cm]{img/gui-screenshots/model.png}
...
\begin{figure}[H]%
  \centering
  \includegraphics[width=12cm]{img/gui-screenshots/processing.png}
...
\begin{figure}[H]%
  \centering
  \includegraphics[width=12cm]{img/gui-screenshots/evaluation.png}
...
Nach Fertigstellung der Berechnung erfolgt eine Darstellung der geschätzten Parameter und Signifikanzen in tabellarischer Form (Abbildung \ref{gui:fig_evaluation}) (\textbf{/MK70/}). Im Falle einer Variationsauswertung werden hier zu jedem Parameter die Signifikanzen aller Varianten auf der horizontalen Achse angezeigt. Die vom Nutzer durch Grenzwertsetzung gewünschten Parameter (unter \emph{View Options} definiert) werden entsprechend hervorgehoben (\textbf{/WK60/}). Am Ende besteht durch einen Klick auf \emph{Export Result} die Möglichkeit, das Ergebnis zu exportieren (\textbf{/MK80/}). Die automatisierte Aktualisierung des zuvor definierten Modells (z.~B. durch Übernehmen der signifikantesten Modellvariante einer Variationsauswertung) kann durch \emph{Update Model} ausgelöst werden (\textbf{/WK70/}).\\
...
\begin{figure}[H]%
  \centering
  \includegraphics[width=6cm,trim={0 12cm 18cm 0},clip]{img/gui-screenshots/menubar1-file.png}
...
\begin{figure}[H]%
  \centering
  \includegraphics[width=6cm,trim={0 8cm 18cm 0},clip]{img/gui-screenshots/menubar2-edit.png}
...
\textbf{Produktfunktion} & \nameref{sec:f:Neues Projekt erstellen}\\ \vspace{1mm}
...
\textbf{Produktfunktion} & \nameref{sec:f:Alternativen importieren} \\
\textbf{Vorbedingung}  & Das Programm ist gestartet. \\ \vspace{1mm} & Die zu importierenden Alternativen sind alle valide. \\
\textbf{Beschreibung}  & Der Nutzer wählt die JSON-Dateien mit den Alternativen aus. \\
\textbf{Nachbedingung} & Die Alternativen wurden importiert und im Programm eingelesen.
\end{tabularx}
\end{table}
...
\textbf{Produktfunktion} & \nameref{sec:f:Alternativen importieren} \\
\textbf{Vorbedingung}  & Das Programm ist gestartet. \\ \vspace{1mm} & Mindestens eine der zu importierenden Alternativen ist invalide oder existiert bereits im Projekt. \\
\textbf{Beschreibung}  & Der Nutzer wählt die JSON-Dateien mit den Alternativen aus. \\ & Das Programm meldet invalide Alternativen in den importierten Dateien. \\ \vspace{1mm}  & Der Nutzer führt den Import fort. \\
\textbf{Nachbedingung} & Die Alternativen wurden importiert und im Programm eingelesen. \\ & Invalide Alternativen werden hervorgehoben.
\end{tabularx}
\end{table}
...
\textbf{Vorbedingung}  & Das Programm ist gestartet. \\ & Mindestens eine der zu importierenden Alternativen ist invalide oder existiert bereits im Projekt.
\vspace{1mm} \\
\textbf{Beschreibung}  & Der Nutzer wählt die JSON-Dateien mit den Alternativen aus. \\ & Das Programm meldet invalide Alternativen in den importierten Dateien. \\ \vspace{1mm}  & Der Nutzer bricht den Import ab. \\
...
\textbf{Produktfunktion} & \nameref{sec:f:Attributsableitungen importieren} \\
\textbf{Vorbedingung}  & Das Programm ist gestartet. \\ \vspace{1mm} & Die zu importierenden Attributsableitungen sind alle valide. \\ \vspace{1mm} 
\textbf{Beschreibung}  & Der Nutzer wählt die JSON-Datei mit den Attributsableitungen aus. \\
\textbf{Nachbedingung} & Die Attributsableitungen wurden importiert und im Programm eingelesen.
\end{tabularx}
\end{table}
...
\textbf{Produktfunktion} & \nameref{sec:f:Attributsableitungen importieren} \\
\textbf{Vorbedingung}  & Das Programm ist gestartet. \\ \vspace{1mm} & Mindestens eine der zu importierenden Attributsableitungen ist invalide oder existiert bereits im Projekt. \\
\textbf{Beschreibung}  & Der Nutzer wählt die JSON-Datei mit den Attributsableitungen aus. \\ & Das Programm meldet invalide Attributsableitungen in der importierten Datei. \\ \vspace{1mm}  & Der Nutzer führt den Import fort. \\
\textbf{Nachbedingung} & Die Attributsableitungen wurden importiert und im Programm eingelesen. \\ & Invalide Attributsableitungen werden hervorgehoben.
\end{tabularx}
\end{table}
...
\textbf{Produktfunktion} & \nameref{sec:f:Attributsableitungen importieren} \\
\textbf{Vorbedingung}  & Das Programm ist gestartet. \\ \vspace{1mm} & Mindestens eine der zu importierenden Attributsableitungen ist invalide oder existiert bereits im Projekt. \\
\textbf{Beschreibung}  & Der Nutzer wählt die JSON-Datei mit den Attributsableitungen aus. \\ & Das Programm meldet invalide Attributsableitungen in der importierten Datei. \\ \vspace{1mm}  & Der Nutzer bricht den Import ab. \\
\textbf{Nachbedingung} & Der Zustand des Programms bleibt unverändert.
\end{tabularx}
\end{table}
...
\textbf{Produktfunktion} & \nameref{sec:f:Alternativen exportieren} \\
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\ \vspace{1mm}  & Es existiert mindestens eine valide Alternative. \\
\textbf{Beschreibung}  & Der Nutzer wählt valide Alternativen aus und exportiert diese. \\
\textbf{Nachbedingung} & Für jede ausgewählte Alternative wurde eine neue JSON-Datei erstellt.
\end{tabularx}
\end{table}
...
\textbf{Produktfunktion} & \nameref{sec:f:Alternativen exportieren} \\
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\  \vspace{1mm}& Es existiert mindestens eine invalide Alternative. \\
\textbf{Beschreibung}  & Der Nutzer wählt Alternativen aus, in denen mindestens eine invalide ist und exportiert diese.  \\ & Das Programm meldet invalide Alternativen. \\ \vspace{1mm} & Der Nutzer führt den Export fort. \\
\textbf{Nachbedingung} & Für jede ausgewählte Alternative wurde eine neue JSON-Datei erstellt.
\end{tabularx}
\end{table}
...
\textbf{Produktfunktion} & \nameref{sec:f:Alternativen exportieren} \\
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\  \vspace{1mm}& Es existiert mindestens eine invalide Alternative. \\
\textbf{Beschreibung}  & Der Nutzer wählt Alternativen aus, in denen mindestens eine invalide ist und exportiert diese.  \\ & Das Programm meldet invalide Alternativen. \\ \vspace{1mm} & Der Nutzer bricht den Export ab. \\
\textbf{Nachbedingung} & Der Zustand des Programms bleibt unverändert.
\end{tabularx}
\end{table}
...
\textbf{Produktfunktion} & \nameref{sec:f:Attributsableitungen exportieren} \\ \vspace{1mm}
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\ \vspace{1mm}
\textbf{Beschreibung}  & Der Nutzer wählt valide Attributsableitungen aus und exportiert diese. \\
\textbf{Nachbedingung} & Für jede ausgewählte Attributsableitung wurde eine neue JSON-Datei erstellt.
\end{tabularx}
\end{table}
...
\textbf{Beschreibung}  & Der Nutzer wählt Attributsableitungen aus, in denen mindestens eine invalide ist und exportiert diese. \\ & Das Programm meldet invalide Attributsableitungen. \\ \vspace{1mm}  & Der Nutzer führt den Export fort. \\
\textbf{Nachbedingung} & Für jede ausgewählte Attributsableitung wurde eine neue JSON-Datei erstellt.
\end{tabularx}
\end{table}
...
\textbf{Produktfunktion} & \nameref{sec:f:Attributsableitungen exportieren} \\ \vspace{1mm}
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\
\textbf{Beschreibung}  & Der Nutzer wählt Attributsableitungen aus, in denen mindestens eine invalide ist und exportiert diese. \\ & Das Programm meldet invalide Attributsableitungen. \\ \vspace{1mm} & Der Nutzer bricht den Export ab. \\
\textbf{Nachbedingung} & Es wurden keine neuen Dateien erstellt.
\end{tabularx}
\end{table}
...
\textbf{Produktfunktion} & \nameref{sec:f:Attributsableitung ändern} \\
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\ \vspace{1mm} & Es existiert mindestens eine Attributsableitung.  \\ \vspace{1mm}
...
\textbf{Produktfunktion} & \nameref{sec:f:Attributsableitung löschen} \\
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\ & Es existiert mindestens eine Attributsableitung.  \\ \vspace{1mm} & Andere Eingaben haben keine Abhängigkeiten zu der ausgewählten Attributsableitung. \\ \vspace{1mm}
...
\textbf{Produktfunktion} & \nameref{sec:f:Attributsableitung löschen} \\
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\ & Es existiert mindestens eine Attributsableitung.  \\ \vspace{1mm} & Andere Eingaben haben eine Abhängigkeit zu der ausgewählten Attributsableitung. \\
...
\textbf{Produktfunktion} & \nameref{sec:f:Attributsableitung löschen} \\
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\ & Es existiert mindestens eine Attributsableitung.  \\ \vspace{1mm} & Andere Eingaben haben eine Abhängigkeit zu der ausgewählten Attrbiutsableitung. \\
...
\\ \vspace{1mm} \textbf{/T50.1/}         & \textbf{Linearkombination} \\
\textbf{Beschreibung}  & Der Nutzer gibt für die Nutzenfunktion eine syntaktisch korrekte Linearkombination ein. (/MK36/) \\
\end{tabularx}
\end{table}
...
\textbf{Produktfunktion} & \nameref{sec:f:Alternative ändern} \\
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\ \vspace{1mm} & Es existiert mindestens eine Alternative.  \\ \vspace{1mm}
...
\textbf{Produktfunktion} & \nameref{sec:f:Berechnung durchführen} \\
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\ & Es existiert mindestens eine Alternative. \\ \vspace{1mm} & Alle Alternativen sind valide. \\ \vspace{1mm}
...
\textbf{Produktfunktion} & \nameref{sec:f:Berechnung durchführen} \\
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\ & Es existiert mindestens eine Alternative. \\ \vspace{1mm} & Es existieren invalide Alternativen. \\
...
\textbf{Produktfunktion} & \nameref{sec:f:Berechnung durchführen} \\
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\ \vspace{1mm} & Es existiert mindestens eine Alternative. \\
...
\textbf{Produktfunktion} & \nameref{sec:f:Schwellwerte konfigurieren} \\
\textbf{Vorbedingung}  & Die CSV-Datei ist im Programm geladen. \\ & Die Berechnungen sind abgeschlossen. \\ \vspace{1mm} & Die Visualisierung der Ergebnisse wird angezeigt.\\ \vspace{1mm}
...
\textbf{Produktfunktion} & \nameref{sec:f:Berechnung durchführen} \\
\textbf{Vorbedingung}  & Alle Dateien wurden geladen. \\ & Es existieren Angaben zum Alter. \\ \vspace{1mm} & Die gewählten Nutzenfunktionen beinhalten das Alter. \\
\vspace{1mm}
...
\textbf{Produktfunktion} & \nameref{sec:f:Variation konfigurieren} \\
\textbf{Vorbedingung}  & Alle Dateien wurden geladen. \\ \vspace{1mm} & Es existiert eine Einteilung der Altersgruppen. \\
\vspace{1mm}
...
\textbf{Produktfunktion} & \nameref{sec:f:Modell optimieren} \\
\textbf{Vorbedingung}  & Alle Dateien wurden geladen. \\ & Berechnungen wurden durchgeführt. \\ & Es existiert eine Einteilung der Altersgruppen. \\ & Es existieren berechnete Signifikanzen. \\ \vspace{1mm} & Eine Konfiguration der Parameter ist eingegeben.\\
\vspace{1mm}
