...
\begin{itemize}
\item Die Software ist eine MVC-Architektur und setzt sich zusammen aus der View, dem Controller und dem Model.
\end{itemize}

\subsection{View}
\begin{itemize}
\item Die View ist verantwortlich für die visuelle Präsentation der im Model vorhandenen Daten. Dafür greift die View auf das externe Paket PyQt zu, welches für die Erstellung graphischer Benutzeroberflächen verwendet wird.
\end{itemize}

\subsection{Controller}
\begin{itemize}
\item Die Controller sind dafür zuständig, die Nutzereingaben aus der View an das Model weiterzuleiten.\\
\item Finden Änderungen am Model statt, so senden die betroffenen Klassen der View update-Aufrufe über die Controller aus. Dabei werden die Daten im Model zurückgegeben.\\
\item Calculation-Paket: Controller, die für die gesamte Funktionalität der Berechnung aufgerufen werden\\
\item Functions-Paket: Controller, die für die Bearbeitung von Alternativen und Attributsableitungen aufgerufen werden
\end{itemize}

\subsection{Model}
\begin{itemize}
\item Das Model umfasst die Datenhaltung bei einem geöffneten Projekt.\\
...
\item Processing-Paket: Umschließt die Funktionalität, welche mit der Konfiguration der Berechnung oder dem Ergebnis zu tun hat. Hier existiert eine Schnittstelle, über welche die tatsächliche Berechnung stattfinden kann. Hier wird standardmäßig das Python Paket \textit{biogeme} verwendet.\\
\item Data-Paket: Bezieht alle Klassen mit ein, die für die Datenhaltung des Modells zuständig sind. \\
\item Das Python Paket \textit{pandas} wird für die Haltung tabellarischer, vom Nutzer nicht veränderbarer Daten verwendet.
\end{itemize}
...
Verwaltet die Daten, die das Verkehrsmodell definieren und wird als Eingabe der Berechnung verwendet. Die Klasse ist die Schnittstelle bei Abfragen und Änderungen des Verkehrsmodells.
\newline \newline

\textbf{{Attribute}}
\begin{itemize}
\item \texttt{data: Data} \newline In das Projekt geladene Erhebungsdaten und Schnittstelle zur Verwaltung der Attributsableitungen.
\item \texttt{alternatives: dict[label: str, expr: FunctionalExpression]} \newline
Definierte Alternativen mit eindeutigem Label.
\\\\
\end{itemize}
...
Verwaltet die Daten der importierten Erhebungsdaten und der Attributsableitungen. Abfragen und Änderungen werden über die Klasse \textit{Model} aufgerufen.
\newline \newline

\textbf{{Attribute}}
\begin{itemize}
\item \texttt{raw\char`_data: DataFrame} \newline Importierte Erhebungsdaten.
\item \texttt{derivatives: dict[label: str, expr: FunctionalExpression]} \newline Von den Erhebungsdaten und/oder anderen Ableitungen definierte Attributsableitungen mit eindeutigem Label.
\\\\
\end{itemize}
...
Repräsentation einer definierten Ableitungsfunktion oder Nutzenfunktion.
\newline \newline
...
Darstellung eines definierbaren Intervalls bei Festlegung der Attributsableitung. Wird bei der Validierung des Funktionsausdrucks und bei der Evaluation erzeugt und kann ausgewertet werden.
\newline \newline
...
Darstellung einer definierbaren Gruppe bei Festlegung der Attributsableitung. Wird bei der Validierung des Funktionsausdrucks und bei der Evaluation erzeugt und kann ausgewertet werden.
\newline \newline

\textbf{Attribute}
\begin{itemize}
\item \texttt{groups: list} \newline Gruppierung aller in der Gruppe enthaltenen Elemente.
\\\\
\end{itemize}
